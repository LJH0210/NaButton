...
void PointsOnCubicBezier(Point2D *cp,int keyFrame,Point2D *cpResult){
    float tSp = 1.0/keyFrame;//帧数间隔
    for (int i = 0; i<keyFrame; i++) {
        float t = i * tSp;
        Point2D point = PointOnCubicBezier(cp, t);
